        ...
        if (set.ReplayLoaded)
        {
            ...
            if (set.ReplayParsed)
            {
                ...
                if (GUILayout.Button("Get player path"))
                {
                    ...
                    List<AdvancedPosition> playerPath = set.getPlayerPathInRound(desiredPlayer, indexRound+1);
                    
                    
                    
                    UnityImporter.combinePlayerWithPath(desiredPlayer, playerPath, colors[desiredPlayer], indexRound + 1);


                    if (saveToCSVToggle)
                    {
                        set.saveToCSV(desiredPlayer, indexRound + 1);
